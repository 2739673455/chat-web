...
import { sendMessage, getUploadPresignedUrl } from '../services/chat'
import { createConversation, updateConversation, generateTitle } from '../services/conversation'
...
export default function ChatInput({ onShowLogin }: ChatInputProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      let displayMessageContent = messageContent // 用于前端显示的消息内容
      ...
      // 同时发送消息和生成标题（如果是新对话）
      const [sendMessagePromise, generateTitlePromise] = isNewConversationCreated
        ? [
            sendMessage(
              {
            ...
            generateTitle({
              ...
              console.log('[generateTitle] Title generated for conversation:', conversationId, 'title:', result.title)
              // 更新对话列表中的标题，或添加新对话
              ...
              if (updatedConversation) {
                ...
                updateConversationStore({ ...updatedConversation, title: result.title })
              } else {
